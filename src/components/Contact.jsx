...
const Contact = () => {
  ...
  return (
      ...
      id="contact"
      className="snap-center min-h-screen overflow-x-hidden"
     >
      <div className="flex flex-col relative h-screen text-center md:text-left  max-w-[900px]  px-3 xl:px-10 justify-center mx-auto items-center ">
       ...
       <h3 className='text-lg sm:text-xl font-semibold mt-16'>I have got just what you need. <span className='underline decoration-[#f7ab0a]/50 text-[#f7ab0a]/50'> Lets Talk</span></h3>
       ...
       <form className='flex flex-col justify-center space-y-2 mt-5 px-2' onSubmit={handleSubmit(onSubmit)}>
        <div className='flex space-x-2'>
            <input {...register("name", { required: true })} type="text" placeholder='Name' className='contact-input' />
            <input {...register("email", { required: true })} type="email" placeholder='Email' className='contact-input' />
        </div>
        <input {...register("subject", { required: true })} type="text" placeholder='Subject' className='contact-input' />
        <textarea {...register("message", { required: true })} placeholder='Message' className='contact-input' rows={5} />

        <button type='submit' className='bg-[#f7ab0a] py-2 md:py-3 px-10 text-neutral-800 rounded-md text-base sm:text-lg font-bold uppercase tracking-[1px]'>Submit</button>
